...
const AddSafetyReportModal = ({ onClose, onSaveNewReport }) => {
  ...
  const handleAddReport = (e) => {
    ...
    if (!selectedProjectName || !reportDate || !agenda.trim()) {
      ...
      messageBox.textContent = 'Please fill out all required fields (Project Name, Report Date, Agenda).';
    ...
    const newReport = {
      ...
      reviewed_at: null,
      // Changed 'description' to 'text' to match the SafetyReport.jsx component's expectation
      activities: [{ text: agenda, images: [] }], // Initial agenda as first activity
    };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center overflow-y-auto modal z-[99999]">
      ...
      >
        ...
        <h2 className="text-xl font-semibold mb-4">Add New Weekly Safety Report</h2>
        ...
        <form onSubmit={handleAddReport} className="space-y-4">
          ...
          <div>
            <label className="block font-medium text-gray-700">Agenda (Initial Activity)</label>
            <textarea
              className="w-full border p-2 rounded-md focus:ring-blue-500 focus:border-blue-500 min-h-[100px]"
              rows={3}
              placeholder="Describe the initial activity for this report"
              value={agenda}
              onChange={(e) => setAgenda(e.target.value)}
              required
            ></textarea>
            {/* Displaying the agenda value */}
            {agenda && (
