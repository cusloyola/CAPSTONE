...
const useIncidentReportHandlers = () => {
  ...
  const submitIncidentReport = async (payload) => {
    try {
      ...
      if (res.ok) {
        ...
        toast.error(data.error || "Failed to submit incident report.");
        return null;
      }
    } catch (err) {
      console.error(err);
  ...
  // ------------------------
  const submitIncidentReportUpdate = async (payload) => {
    try {
      const formPayload = new FormData();
      ...
      const res = await fetch(
        `http://localhost:5000/api/incidentReports/${payload.incident_report_id}`,
        {
      ...
      if (res.ok) {
        ...
        toast.error(data.error || "Failed to update incident report.");
        return null;
      }
    } catch (err) {
      console.error(err);
      toast.error("Error updating incident report.");
      return null;
    }
  };
...
export default useIncidentReportHandlers;
