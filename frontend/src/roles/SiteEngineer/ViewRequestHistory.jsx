...
    const [isExporting, setIsExporting] = useState(false); // New state to control rendering
    const pdfRef = useRef();
...
    // Fetch and process data from the backend
    useEffect(() => {
...
                // Process each request to parse the 'items' string
...
    const requestsToExport = filteredRequests.filter(r => selectedRequests.includes(r.request_id));
    ...
    const handleExport = () => {
        ...
        setIsExporting(true); // Start the export process by rendering the hidden component
        setOpenExportMenu(false); // Close the menu

        // Wait for the component to render, then export and hide it again
        setTimeout(() => {
            exportMaterialRequestPDF(pdfRef);
            setIsExporting(false); // Hide the component after export
        }, 100);
    };
...
                    </div>
                </div>
                {/* Conditionally render the PDF component */}
                {isExporting && (
