...
const DeleteModalLUC = ({ data, onClose, onDelete, labor_entry_id }) => {
    const [success, setSuccess] = useState(false);

    if (!labor_entry_id) return null;

    const handleConfirmDelete = async () => {
        console.log('Attempting to delete labor_entry_id ', labor_entry_id);

        try {
            const res = await fetch(`http://localhost:5000/api/laborunitcost/delete/${labor_entry_id}`, {
                method: 'DELETE',
            });
            ...
            if (!res.ok) {
                ...
                console.error('Failed to delete Labor Unit Cost:', errorBody);
                throw new Error('Failed to delete Labor Unit Cost');
            }
