...
export default function ProjectInfoDetailedCard({ project_id, onProjectUpdate }) {
    ...
    const fetchProjectDetails = async () => {
      ...
      try {
        ...
        if (isMounted.current) {
          ...
          setEditClientContact(data.client_contact || '');
          setEditProjectCategory(data.projectCategory || '');
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    const hasChanged =
      ...
      editClientName !== projectData.client_name ||
      editClientContact !== projectData.client_contact ||
      editProjectCategory !== projectData.projectCategory;
    ...
    try {
      const response = await fetch(`${PROJECTS_API_URL}${project_id}`, {
        method: 'PUT',
        headers: {
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Project ID</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{projectData.project_id}</p>
              </div>
              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Client Name</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{projectData.client_name || 'N/A'}</p>
              </div>
              <div>
              ...
              {/* Add more detailed project fields here */}
              <div>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{projectData.ProjectManager}</p>
              </div>
              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Client Contact</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{projectData.client_contact || 'N/A'}</p>
              </div>
              <div>
                <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Project Category</p>
                <p className="text-sm font-medium text-gray-800 dark:text-white/90">{projectData.projectCategory || 'N/A'}</p>
              </div>
            </div>
          </div>
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
              Edit Detailed Project Information
            </h4>
