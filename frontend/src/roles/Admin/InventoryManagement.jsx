...
const InventoryManagement = () => {
    ...
    const deleteItem = async (itemId) => {
      try {
          ...
          await fetchInventory(); //Re-fetch the inventory after a delete.
      } catch (error) {
  ...
  const confirmDelete = async () => {
    ...
    const fetchInventory = async () => {
      ...
      try {
          ...
          console.log("Inventory state after setInventory:", response.data); // Added log here
      } catch (error) {
                    ...
                    <tbody>
                        {inventory.map((item) => (
                            <tr key={item.item_id} style={{ borderBottom: "1px solid #ddd" }}>
                                ...
                                <td style={{ padding: "10px" }}>{item.stock_quantity}</td><td style={{ padding: "10px" }}>{item.reorder_level}</td>
